...
export default function AdminActionsPage() {
    const [adminWallets] = useState<string[]>(() => {
      if (typeof window !== 'undefined') {
        return (process.env.NEXT_PUBLIC_ADMIN_WALLETS || '').split(',').map(a => a.toLowerCase().trim());
      }
      return [];
    });
    ...
    filtered = filtered.sort((a, b) => {
      const va = (a as AdminAction)[sortBy as keyof AdminAction] || "";
      const vb = (b as AdminAction)[sortBy as keyof AdminAction] || "";
    ...
    // Restrict page to admins only (render conditionally)
