...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (DEBUG_REPORTS) console.log('[/api/reports] incoming POST');
  ...
  if (DEBUG_REPORTS) {
    console.log('[/api/reports] body:', {
      reporterWalletId,
      providerId,
      suspectAddress,
      chain,
      evidenceLength: Array.isArray(evidence) ? evidence.length : undefined,
    });
  }
  if (!suspectAddress || !chain) {
    if (DEBUG_REPORTS) console.log('[/api/reports] missing suspectAddress or chain');
    return res.status(400).json({ message: 'suspectAddress and chain required' });
  }
  await dbConnect();
  // optionally ensure wallet belongs to user
  if (reporterWalletId) {
    if (DEBUG_REPORTS) console.log('[/api/reports] validating reporterWalletId:', reporterWalletId);
    const w = await Wallet.findById(reporterWalletId);
    if (!w) {
      if (DEBUG_REPORTS) console.log('[/api/reports] reporter wallet not found:', reporterWalletId);
      return res.status(400).json({ message: 'reporter wallet not found' });
    }
  }
  // if providerId is provided but doesn't exist, reject
  if (providerId) {
    if (DEBUG_REPORTS) console.log('[/api/reports] validating providerId:', providerId);
    const p = await Provider.findById(providerId);
    if (!p) {
      if (DEBUG_REPORTS) console.log('[/api/reports] provider not found:', providerId);
