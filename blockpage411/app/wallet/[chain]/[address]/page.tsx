...
export default function WalletProfile() {
  ...
  if (isLoading) return <div className="p-8 text-center text-lg text-gray-600" aria-busy="true">Loading wallet profile...</div>;
  if (error) return <div className="p-8 text-center text-red-600 font-semibold" role="alert">Error loading wallet</div>;
  if (!data) return <div className="p-8 text-center text-gray-400">No wallet data found.</div>;

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex flex-col">
      <Navbar variant="wallet" />
      <main className="flex-1 flex items-center justify-center py-4 px-2 sm:px-0">
        <section className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-200 p-4 sm:p-8 flex flex-col gap-8 animate-fade-in">
          {/* Wallet Header */}
          <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-100 pb-4 w-full">
            <div className="min-w-0 flex-1">
              <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 flex items-center gap-2 break-all sm:break-words truncate" aria-label="Wallet address">
                <span className="text-blue-600 text-3xl" aria-hidden="true">👛</span>
              ...
              {data.ens && (
                <div className="text-blue-700 text-xs mt-1" aria-label="ENS name">ENS: {data.ens}</div>
              )}
              {typeof data.nftCount === 'number' && data.chain === 'ethereum' && (
                <div className="text-purple-700 text-xs mt-1" aria-label="NFT count">NFTs: {data.nftCount}</div>
              )}
            </div>
            <div className="flex flex-col items-end flex-shrink-0">
              <span className="text-yellow-500 text-lg font-bold flex items-center gap-1" aria-label="Average rating">
          ...
          <section className="flex flex-col gap-2" aria-labelledby="flags-heading">
            <h2 id="flags-heading" className="font-semibold text-lg text-red-600 flex items-center gap-2 mb-1">
          ...
          <section className="flex flex-col gap-2" aria-labelledby="rating-heading">
            <h2 id="rating-heading" className="font-semibold text-lg text-yellow-600 flex items-center gap-2 mb-1">
          ...
          <section className="flex flex-col gap-2" aria-labelledby="tx-heading">
            <h2 id="tx-heading" className="font-semibold text-lg text-blue-700 flex items-center gap-2 mb-1">
              <span aria-hidden="true">📄</span> Recent Transactions
            </h2>
            <ul className="max-h-56 overflow-y-auto divide-y divide-gray-100 bg-gray-50 rounded-lg border border-gray-100" aria-live="polite">
              {data.transactions?.length ? data.transactions.slice(0, 10).map((tx: any, i: number) => (
                <li key={i} className="text-xs px-3 py-2 flex flex-col sm:flex-row sm:items-center gap-1">
                  {data.chain === 'bitcoin' ? (
                    <>
                      <span className="font-mono text-gray-700">Txid: {tx.txid?.slice(0, 10)}...</span>
                      <span className="text-gray-400">Block: {tx.status?.block_height || 'N/A'}</span>
                      <span className="text-blue-700 font-semibold">{tx.vout?.length || 0} outputs</span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-blue-700 font-semibold">{Number(tx.value) / 1e18} {data.chain === 'polygon' ? 'MATIC' : data.chain === 'bsc' ? 'BNB' : 'ETH'}</span>
                    </>
                  )}
                </li>
